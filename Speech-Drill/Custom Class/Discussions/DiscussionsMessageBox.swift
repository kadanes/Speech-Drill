...
class DiscussionsMessageBox: UIView {
    ...
    override init(frame: CGRect) {
        logger.info("Initializing discussion message box")
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(endEditingOnChatViewTap), name: NSNotification.Name(chatViewTappedNotificationName), object: nil)
    }
...
extension DiscussionsMessageBox {
    ...
    func updateSendButton() {
        logger.debug("Toggling send button state")
    ...
    @objc func endEditingOnChatViewTap(_ notification: NSNotification) {
        logger.info("Ending editing when chat view tapped")
        messageTextView.endEditing(false)
    }
    
    @objc func sendMessageButtonPressed(_ sender: UIButton) {
        logger.event("Tapped send message button")
        ...
        if currentUser == nil {
            ...
            do {
                ...
                logger.event("Sent message \(messageDictionary)")
                
                newMessageReference.setValue(messageDictionary) { (error, reference) in
                    if let error = error {
                        logger.error("Failed to save message \(messageDictionary) in database: \(error)")
                    }
                }
                askForReview()
                reregisterForTopicFCM()
            } catch {
                logger.event("Failed to parse message \(message) to dictionary")
            }
    ...
    func reregisterForTopicFCM() {
        logger.info("Reregistering for topic notifications")
